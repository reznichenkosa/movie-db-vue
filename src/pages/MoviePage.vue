<template>
  <section class="movie">
    <div
      class="background"
      :style="{background: `url(${movie.coverUrl || require('@/assets/img/cover-default.png')}) center top/cover no-repeat`}"
    ></div>
    <div class="container">
      <div class="wrapper">
        <div class="content">
          ...
          <ul class="list">
            ...
            <li v-for="genre in movie.genres" :key="genre">{{genre.genre}}</li>
            <li v-if="movie.year">{{movie.year}}</li>
            <li v-if="movie.filmLength">{{formatFilmLength(movie.filmLength)}}</li>
            <li v-if="movie.ratingAgeLimits">{{movie.ratingAgeLimits && movie.ratingAgeLimits.replace('age', '')}}+</li>
          </ul>
